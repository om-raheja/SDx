...
test('test that submission counting works correctly', async ({ page }) => {
  ...
  // If we didn't find a good candidate, just use the first one
  if (!testCaseButton) {
    testCaseButton = page.locator('button:has-text("View Submissions")').first();
    // Get its text to report
    const firstButtonText = await testCaseButton.textContent();
    console.log('Using first submission button:', firstButtonText);
  }
  ...
  // Find the case container for this button to click on the case title
  if (testCaseIndex >= 0) {
    ...
    if (testCaseIndex < caseContainers.length) {
      ...
      const caseTitle = selectedContainer.locator('span.text-lg.font-medium');
