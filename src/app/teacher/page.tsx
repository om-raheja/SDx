              ...
              return Object.values(grouped).map((g: any) => (
                <div key={`${g.email}-${g.case_id}`} className="p-4 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800">
                  ...
                  <div className="space-y-2 mb-4">
                    {g.submissions.map((sub: any) => (
                      <div key={sub.id} className="flex items-start gap-2 text-sm p-2 rounded bg-zinc-50 dark:bg-zinc-800">
                        <span className="px-2 py-0.5 bg-zinc-200 dark:bg-zinc-700 rounded text-xs font-medium">H{sub.submitted_after_hint}</span>
                        <span className="text-zinc-700 dark:text-zinc-300">{sub.diagnosis || 'pending'}</span>
                      </div>
                    ))}
                  </div>
